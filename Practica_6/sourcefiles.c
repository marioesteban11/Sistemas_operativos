...
struct cantidad
analizar_fichero(char * fichero, struct stat SMetaData, struct cantidad *cantidad)
{
    char * final_fichero;
    char punto = '.';
...
read_directory(char *directorio, struct cantidad *cantidad)
{
	...
	while ((direntp = readdir(dirp)) != NULL) {  
        ...
        if (direntp->d_name[0] != '.'){
            fichero_final = (char *)malloc((strlen(direntp->d_name) + strlen(directorio) )*sizeof(char*) );  
            sprintf(fichero_final, "%s/%s", directorio, direntp->d_name);
            ...
            if (stat(fichero_final, &SMetaData) == -1) {
                ...
                if ((SMetaData.st_mode & S_IFMT) == S_IFREG) {
                    analizar_fichero(direntp->d_name, SMetaData, cantidad);
                }else if ((SMetaData.st_mode & S_IFMT) == S_IFDIR){
...
encontrar_directorios(){
    ...
    while (fgets(bufer, SIZE, stdin))
    {
        ...
        printf("%s\t%d\t%d\t%d\n", bufer, cantidad.ficheros_c, cantidad.ficheros_h, cantidad.bytes);
    }
...
//strrchr usar para averiguar que "extension" tiene por ejemplo f1.work.c te da un puntero al ultimo caracter que has puesto.
// Buscar strrchr(string, ".") te da un puntero a .c
int
